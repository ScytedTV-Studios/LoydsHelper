...
function writeStatusFile() {
    updateQueue = updateQueue.then(() => {
        return new Promise((resolve) => {
            fs.writeFile(path, JSON.stringify(onlineUsers, null, 2), (err) => {
                if (err) console.error("Error writing status file:", err.message);
                resolve();
            });
        });
    });
    ...
    if (!onlineUsers[userId] || onlineUsers[userId].timestamp === null) {
        ...
        if (highestBadge && announceChannel) {
            const embed = new EmbedBuilder()
                .setColor("Red")
                .setDescription(`<:crossmark:1330976664535961753> <@${userId}> has lost the <@&${highestBadge}> badge.`);
